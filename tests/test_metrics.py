...
def test_domain_metrics_counters_increment() -> None:
    ...
    assert (
        DROP_UPLOADS_TOTAL.labels(status="success")._value.get() == initial_uploads + 1
    )
    ...
    initial_rejected = DROP_DOWNLOADS_REJECTED_TOTAL.labels(
        reason="expired"
    )._value.get()
    DROP_DOWNLOADS_REJECTED_TOTAL.labels(reason="expired").inc()
    assert (
        DROP_DOWNLOADS_REJECTED_TOTAL.labels(reason="expired")._value.get()
        == initial_rejected + 1
    )


def test_celery_metrics_counters_increment() -> None:
    initial_failures = CELERY_TASK_FAILURES_TOTAL.labels(
        task_name="drop.delete_file"
    )._value.get()
    CELERY_TASK_FAILURES_TOTAL.labels(task_name="drop.delete_file").inc()
    assert (
        CELERY_TASK_FAILURES_TOTAL.labels(task_name="drop.delete_file")._value.get()
        == initial_failures + 1
    )

    initial_retries = CLEANUP_RETRIES_TOTAL.labels(
        task_name="drop.delete_file"
    )._value.get()
    CLEANUP_RETRIES_TOTAL.labels(task_name="drop.delete_file").inc()
    assert (
        CLEANUP_RETRIES_TOTAL.labels(task_name="drop.delete_file")._value.get()
        == initial_retries + 1
    )
